...
- Keep full name, email, and phone in a protected runtime state; the bundled ledger intentionally has no direct customer-contact columns.
- Give the ledger only a pseudonymous `conv_` HMAC-SHA256 key generated with a deployment secret—never a phone number, email, or unsalted hash. Treat that key as personal data for access and retention purposes.
